...
class CGCSA(nn.Module):
  ...
  def forward(self, x):
      ...
      x = self.CNN_extract(x) # feature extract
      x = x.view(batch_size, seq_length, self.hidden_channels, self.flat, self.flat)
      
      #feature maps to graph
      x, batch_size, seq, c, h, w = process_batch_to_graph(x, add_temporal_edges=False, add_future_frame=False)
